...
for column in idinput:
    trend = signal.convolve(idinput[column], coef, mode='same')
    trend[:numtaps] =  trend[numtaps+1]
    trend[-numtaps:] = trend[-numtaps-1]
...
start_time = start
end_time = stop
plots.plot_comparison(step_test_data, model, inputs, outputs, start_time, end_time, plt_input=True ,scale_plt=False)